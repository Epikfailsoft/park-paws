...
export default function Onboarding() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setOwnerLastName(profile.last_name || '');
    }
  }, [profile]);

  useEffect(() => {
    supabase.from('breeds').select('*').order('name').then(({ data }) => { if (data) setBreeds(data as Breed[]); });
  }, []);

  useEffect(() => {
    supabase.from('parks').select('id, name, status').eq('status', 'ACTIVE').order('name').then(({ data }) => { if (data) setParks(data); });
  }, []);

  const filteredBreeds = breeds.filter(breed => breed.name.toLowerCase().includes(breedSearch.toLowerCase()));
  ...
  const handlePhotoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      } catch { toast.error('Fotoğraf işlenemedi'); }
    }
  };

  const handleSaveOwnerName = async () => {
    if (!profile || !ownerDisplayName.trim()) return;
    setSavingName(true);
    try {
      await supabase.from('profiles').update({ display_name: ownerDisplayName.trim(), last_name: ownerLastName.trim() || null } as any).eq('id', profile.id);
  ...
  const handleDogSubmit = async () => {
    ...
    if (!photo) { toast.error('Lütfen köpeğinin bir fotoğrafını yükle'); return; }
  ...
  const handleParkSelect = async () => {
    if (!selectedParkId) { toast.error('Lütfen bir park seç'); return; }
    setLoading(true);
    try {
      ...
      <div className="flex min-h-screen flex-col bg-background safe-top">
        ...
        <div className="px-6 pt-4 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-xl font-bold text-foreground">Köpeğini Tanıyalım</h1>
              <p className="text-sm text-muted-foreground">Parkta tanışmaya hazır olsun</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-24">
          ...
          <div className="mb-6">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()}
              className={cn("relative mx-auto flex aspect-square w-40 items-center justify-center overflow-hidden rounded-3xl border-2 border-dashed transition-all",
          ...
          {/* Name */}
          <div className="mb-4">
            <label className="mb-1.5 block text-sm font-medium text-foreground">Köpeğin Adı *</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Luna, Max, Boncuk..."
              className={cn("dogspace-input w-full", errors.name && "border-destructive")} />
          ...
          <div className="mb-4 relative">
            ...
            <button type="button" onClick={() => setShowBreedDropdown(!showBreedDropdown)} className="dogspace-input w-full text-left flex items-center justify-between">
              <span className={selectedBreed ? "text-foreground" : "text-muted-foreground"}>{selectedBreed?.name || "Irk seçin"}</span>
          ...
          {selectedBreed?.code === 'OTHER' && (
            <div className="mb-4">
              <label className="mb-1.5 block text-sm font-medium text-foreground">Hangi kırma/melez? *</label>
              <input type="text" value={customBreedText} onChange={(e) => setCustomBreedText(e.target.value)} placeholder="Örn: Golden + Husky karışımı" className="dogspace-input w-full" />
            </div>
          )}

          {/* Age */}
          <div className="mb-4">
            <label className="mb-1.5 block text-sm font-medium text-foreground">Yaklaşık Yaş *</label>
            <input type="text" value={age} onChange={(e) => setAge(e.target.value)} placeholder="~2 yaş, 6 aylık, 5 yaş..."
              className={cn("dogspace-input w-full", errors.approximate_age && "border-destructive")} />
            {errors.approximate_age && <p className="mt-1 text-xs text-destructive">{errors.approximate_age}</p>}
          </div>

          {/* Energy Level */}
          <div className="mb-6">
            <label className="mb-3 block text-sm font-medium text-foreground">Enerji Seviyesi *</label>
              ...
              {([1, 2, 3] as const).map((level) => (
                <button key={level} type="button" onClick={() => setEnergyLevel(level)}
                  className={cn("flex-1 rounded-xl border-2 py-3 text-sm font-medium transition-all",
                    energyLevel === level ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/50"
          ...
          {/* Neutered */}
          <div className="mb-6">
            <label className="mb-3 block text-sm font-medium text-foreground">Kısırlaştırıldı mı? *</label>
            <div className="flex gap-3">
              {[true, false].map(val => (
                <button key={String(val)} type="button" onClick={() => setNeutered(val)}
                  className={cn("flex-1 rounded-xl border-2 py-3 text-sm font-medium transition-all",
          ...
          {/* Dog Origin */}
          <div className="mb-6">
            <label className="mb-3 block text-sm font-medium text-foreground">Köpeğin nereden geldi?</label>
            <div className="flex gap-2">
              {([
                { value: 'none' as const, label: 'Sahipli', icon: '🐕' },
                { value: 'shelter' as const, label: 'Barınak', icon: '🏠' },
                { value: 'street' as const, label: 'Sokak', icon: '🐾' },
              ]).map(opt => (
                <button key={opt.value} type="button"
                  onClick={() => { setDogOrigin(opt.value); setIsShelter(opt.value !== 'none'); }}
                  className={cn("flex-1 rounded-xl border-2 py-3 text-xs font-medium transition-all",
                    dogOrigin === opt.value
                      ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground"
          ...
          <div className="mb-6">
            ...
            <input type="tel" value={emergencyPhone} onChange={(e) => setEmergencyPhone(e.target.value)} placeholder="0555 123 45 67"
              className={cn("dogspace-input w-full", errors.emergency_phone && "border-destructive")} />
            <p className="mt-1 text-xs text-muted-foreground">🔒 Sadece Lost Mode aktifken görünür</p>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border p-4 safe-bottom">
          <button type="button" onClick={handleDogSubmit} disabled={loading}
            ...
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Köpeğimi Kaydet <ArrowRight className="h-5 w-5" /></>}
          </button>
        </div>
      </div>
    );
    ...
    <div className="flex min-h-screen flex-col bg-background safe-top">
      ...
      <div className="px-6 pt-4 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-xl font-bold text-foreground">Parkını Seç</h1>
            <p className="text-sm text-muted-foreground">Köpeğinle buluşma noktası</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 px-6 pb-24">
        <div className="space-y-3">
          {parks.map((park) => (
            <button key={park.id} type="button" onClick={() => setSelectedParkId(park.id)}
              className={cn("w-full rounded-2xl border-2 p-4 text-left transition-all",
                selectedParkId === park.id ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/50"
              )}>
              <div className="flex items-center gap-3">
                ...
                {selectedParkId === park.id && (
                  <div className="h-6 w-6 rounded-full bg-primary flex items-center justify-center"><span className="text-primary-foreground text-sm">✓</span></div>
                )}
              </div>
            </button>
          ))}
        </div>
        {parks.length === 0 && <div className="text-center py-8"><p className="text-muted-foreground">Henüz aktif park yok</p></div>}
      </div>

      <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border p-4 safe-bottom">
        <button type="button" onClick={handleParkSelect} disabled={loading || !selectedParkId}
          ...
          {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Başla <ArrowRight className="h-5 w-5" /></>}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background safe-top">
      ...
      <div className="flex-1 px-6">
        {/* Owner Photo Upload */}
        <div className="mb-6">
          <input ref={ownerFileInputRef} type="file" accept="image/*" onChange={handleOwnerPhotoSelect} className="hidden" />
          <button type="button" onClick={() => ownerFileInputRef.current?.click()}
            className={cn("relative mx-auto flex h-32 w-32 items-center justify-center overflow-hidden rounded-3xl border-2 border-dashed transition-all",
              ownerPhotoPreview ? "border-primary" : "border-border hover:border-primary/50"
            )}>
            {ownerPhotoPreview ? (
              <>
                <img src={ownerPhotoPreview} alt="Preview" className="h-full w-full object-cover" />
                <div className="absolute inset-0 flex items-center justify-center bg-foreground/20 opacity-0 transition-opacity hover:opacity-100"><Camera className="h-8 w-8 text-white" /></div>
              </>
            ) : (
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
        ...
        {/* Owner Name - Editable */}
        <div className="mb-6 rounded-xl bg-card p-4" style={{ boxShadow: 'var(--shadow-card)' }}>
          {editingName ? (
            <div className="space-y-3">
              <div>
                <label className="text-xs text-muted-foreground mb-0.5 block">Ad</label>
                <input type="text" value={ownerDisplayName} onChange={(e) => setOwnerDisplayName(e.target.value)}
                  className="dogspace-input w-full text-sm" placeholder="Adınız" />
              </div>
              <div>
                ...
                <input type="text" value={ownerLastName} onChange={(e) => setOwnerLastName(e.target.value)}
                  className="dogspace-input w-full text-sm" placeholder="Soyadınız" />
              </div>
              <div className="flex gap-2">
                <button onClick={() => { setEditingName(false); setOwnerDisplayName(profile?.display_name || ''); setOwnerLastName(profile?.last_name || ''); }}
                  className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground">İptal</button>
                <button onClick={handleSaveOwnerName} disabled={savingName || !ownerDisplayName.trim()}
                  className="rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground disabled:opacity-50">
                  {savingName ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Kaydet'}
                </button>
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setEditingName(true)} className="flex items-center gap-1 text-xs text-primary font-medium">
        ...
        <button
          onClick={() => { if (ownerPhoto) handleOwnerPhotoUpload().then(() => navigate('/discover')); else navigate('/discover'); }}
          disabled={ownerPhotoLoading}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3.5 font-medium text-primary-foreground transition-all hover:opacity-90 disabled:opacity-50">
          {ownerPhotoLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Başla <ArrowRight className="h-5 w-5" /></>}
        </button>
