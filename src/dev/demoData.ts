...
//
// Nothing runs at import time: production builds import this module but never call
// getDemoData(), and that lets the bundler drop the whole file.

export interface DemoData {
  user: User;
  profile: Profile;
  dogs: Dog[];
  discoverDogs: DiscoverDog[];
  parkDogs: ParkDog[];
}
...
function buildDemoData(): DemoData {
  ...
  const discoverDogs: DiscoverDog[] = [
    discoverDog({
  ...
  const parkDogs: ParkDog[] = discoverDogs.map((dog, i) => ({
    dog_id: dog.dog_id,
  ...
  return { user, profile, dogs, discoverDogs, parkDogs };
}
